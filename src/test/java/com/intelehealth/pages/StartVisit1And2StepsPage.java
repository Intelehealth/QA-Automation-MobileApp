...
// 2/4 visit reason
	...
	@AndroidFindBy(id = "org.intelehealth.app:id/tv_selected_reason_lbl")
	private WebElement selectedReasonLabel;

	@AndroidFindBy(id = "org.intelehealth.app:id/tv_all_reason_lbl")
	...
	@AndroidFindBy(id = "org.intelehealth.app:id/prog_bar_step3")
	private WebElement progressBar2;

	@AndroidFindBy(xpath = "//android.widget.RelativeLayout[@content-desc='selected_list_item_0']/android.widget.TextView[@resource-id='org.intelehealth.app:id/tv_name']")
...
// 10 of 12
	...
	@AndroidFindBy(id = "org.intelehealth.app:id/btn_view_date")
...
// Other Details Section
	...
	@AndroidFindBy(id = "org.intelehealth.app:id/tv_no")
	private List<WebElement> lstNoBtnInAssoSymptoms;
...
//selects a date
	...
	public String getSelectedMenstruationDate() {
		return selectedMenstruationDate.getText();
	}
...
//+ "new UiSelector().description(\"Associated Symptoms Questionnaire Main View Submit Button\")):"));
	...
	public boolean verifyUserAllowedToSelectOneOptionInElevenOfTwelveQuestion() throws InterruptedException {
		click(yesDescribeOption);
		scrollDown();
		boolean b = isDisplayed(describeTextField);
		click(noneOption);
...
//method to verify 2/4 visit reason page
	...
	public void selectAssociatedSymptoms() {
		/*
		 * for(int i =0; i<lstNoBtnInAssoSymptoms.size();i++) { if(i==5) { scrollDown();
		 * i=0; continue; } lstNoBtnInAssoSymptoms.get(i).click();
		 * 
		 * }
		 */
		...
		while (i < lstNoBtnInAssoSymptoms.size()) {
			lstNoBtnInAssoSymptoms.get(i).click();
			i++;

			// Every 5 clicks, scroll down to reveal the next batch of items
			if (i % 5 == 0) {
				int sizeBeforeScroll = lstNoBtnInAssoSymptoms.size();
				scrollDown();

				// If scrolling didn't reveal any new elements, we've hit the bottom — stop
				if (lstNoBtnInAssoSymptoms.size() == sizeBeforeScroll) {
					break;
				}
			}
		}
